...
function sellNftWithEth(uint256 tokenId,uint256 price)public payable nonReentrant{//mode==0 sell nft with eth  mode==1 sell nft with abcoin
require(nft.ownerOf(tokenId)==msg.sender,"you have not access to this nft");
...
listItems[ItemIdCountable]=List(ItemIdCountable,tokenId,payable(msg.sender),payable (address(this)),price,false,_image);
...
emit NftListCreated(ItemIdCountable, tokenId, msg.sender, address(this), price, false);
}

function sellNftWithAbCoin(uint256 tokenId,uint256 price)public payable nonReentrant{//mode==0 sell nft with eth  mode==1 sell nft with abcoin
require(nft.ownerOf(tokenId)==msg.sender,"you have not access to this nft");
require(price>0,"price should be higher than 0");
require(msg.value==marketFee,"please transfer fee price");
ItemIdCountableAbCoin++;
string memory _image=nft.tokenURI(tokenId);
listItemsAbcoin[ItemIdCountableAbCoin]=List(ItemIdCountableAbCoin,tokenId,payable(msg.sender),payable (address(this)),price,false,_image);
nft.transferFrom(msg.sender,address(this),tokenId);//the seller sends his nft to the contract to be sold*****(owner of nft should approve this contract address to sell his nft!you can do this in nft contract with approveall function)*****
emit NftListCreated(ItemIdCountable, tokenId, msg.sender, address(this), price, false);
}
...
//Auction
function createAuction(uint256 tokenId,uint256 auctionFinishTimeUnix)public payable nonReentrant {
require(nft.ownerOf(tokenId)==msg.sender,"you have not access to this nft");
require(msg.value==marketFee,"please transfer fee price");
require(auctionFinishTimeUnix>block.timestamp,"auction finish time should be higher than current unix time");
ItemIdCountable++;
...
emit NftAuctionCreated(ItemIdCountable, tokenId, msg.sender, address(this), 0,address(this), auctionFinishTimeUnix, false);
}
...
require(selectedItem.finishTime>block.timestamp,"sorry...The auction is over");
...
function calculateAuctionResult(uint256 _itemId)public payable nonReentrant{
auctionStruct storage selectedItem=auctionItems[_itemId];
require(selectedItem.finishTime<block.timestamp,"auction time is not ended");
...
require(token.balanceOf(msg.sender)>=selectedItem.price,"insufficient token!");
...
//in this function, we get the list of store nfts AB
 function getNftItemsAB() public view returns (List[] memory) {
    uint256 nftCount = nft.totalSupply();
    uint currentIndex = 0;
    List[] memory items = new List[](nftCount);
    for (uint i = 0; i < nftCount; i++) {
        if (listItemsAbcoin[i + 1].owner == address(this)) {
        uint currentId = i + 1;
        List storage currentItem = listItemsAbcoin[currentId];
        items[currentIndex] = currentItem;
        currentIndex += 1;
      }
    }
